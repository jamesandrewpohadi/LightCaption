...
def get_glove_embedding(vocab,glove_file,dim):
    lines = open('glove.6B.{}d.txt'.format(dim)).read().splitlines()
    glove_embedding = {}
    for line in lines:
        token = line.split()
        word = token[0]
    ...
    torch.save(e.state_dict(),'coco2014.glove.6B.{}d.pth'.format(dim))
    print('saved glove embedding in coco2014.glove.6B.{}d.pth'.format(dim))
